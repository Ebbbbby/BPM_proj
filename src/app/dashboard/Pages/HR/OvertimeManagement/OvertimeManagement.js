...
import { FormatDateTime, GetSearchParams } from "../../../../../utils/functions/ResourceFunctions";
...
import {ApproveOvertimeRequest ,DeclineOvertimeRequest,GetOvertimeType, GetAllOvertimeRequisition,UpdateOvertime } from '../../../../../utils/redux/HR/HRSlice';
import { FormatCurrency } from '../../../../../utils/functions/FormatCurrency';
import ApprovalData from '../ApprovalConstants/data'
import { FormInput, FormSelect, FormTemplate } from '../../../Components/Forms/InputTemplate';
...
function OvertimeRequisition() {
    ...
    const { get_all_overtime, isLoading ,get_overtime_type } = useSelector((state) => state?.hr);
    const appURL = new URLSearchParams(window.location.search);
    const getURLData = (e) => appURL.get(e);
  
    const dispatch = useDispatch();
  
    let overTime = get_all_overtime?.data;  
    overTime = overTime?.filter(row => row.initiatorName.toLowerCase().includes(search.toLowerCase()));
    ...
    useEffect(() => {
      ...
      dispatch(
        GetAllOvertimeRequisition(apiData)       
      );
    ...
    const handleFilterButton = () => {
      dispatch(GetAllOvertimeRequisition(apiData));
    };
    ...
    return (
      <PageStyle
        title={"All Overtime Requisition"}
        hasBack={false}
        action={actionButton}
        isMain={true}
      >
        ...
        <Pagination
          last_page={get_all_overtime?.metaData?.totalPages}
          present_page={get_all_overtime?.metaData?.page}
          totalRows={get_all_overtime?.metaData?.perPage}
          pageSize={pageSize}
    ...
    return (
      <>
        <tr key={id}>
          ...
          <td>
            {approvalStatus === 'Pending'? (  <TableActions hasChildren={true} url={`${id}/view`} >
              {[
                  {
                    name: "Edit Overtime Requisition",
                    action: () => {
                        setIsOpen(!isOpen);
                        navigate(`?modal_type=Edit&isEdit=true&assignment=${assignment}&date=${overtimeDate}&id=${id}&overtimeType=${overtimeType}&initiator=${initiatorId}&hours=${numberOfHours}`);
                    },
                  },       
                  
                  {
                    name: "Approve Overtime Requisition",
                    action: () => {
                      openModal({
                        type: "suspend",
                        details: {
                          ...
                          submitData: (data) => {
                            ...
                            dispatch(
                              ApproveOvertimeRequest({
                                ...
                                console.log(res)
                              closeModal();
                              dispatch(GetAllOvertimeRequisition({pageNumber:1, pageSize:1000}));
                            });
                          },
                        },
                      });
                  ...
                  {
                    name: "Decline Overtime Requisition",
                    action: () => {
                      openModal({
                        type: "suspend",
                        details: {
          ...
          </TableActions> ): ( <TableActions hasChildren={true} url={`${id}/view`}></TableActions> )}
  ...
  export function CreateSetUpActions({
    ...
    const submit = (data) => {
     ...
     // return
        ...
        //     }
        //   })}
        if (GetSearchParams("isEdit") === "true") {
          dispatch(
            UpdateOvertime({
              ...data,
              requisitionId: GetSearchParams("id"),
    ...
    return (
      <div className={DashboardStyle.dash_board_home_nav}>
        <div className={DashboardStyle.dash_board_home_nav_header}>
          <h4>
            {GetSearchParams("isEdit") && (' Overtime Requisition') }
          </h4>
          <FiX
            ...
            <FormProvider {...formMethods}>
              <FormTemplate handleSubmit={handleSubmit(submit)}>
                  ...
                  array={overtimeType?.data?.map((types,index)=>(
                    <option key={index} value={+types?.id}>{types?.title}</option>
                  ))}
                />
                <div>
