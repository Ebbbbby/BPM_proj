...
import { UpdateQuerySetup } from "../../../../../utils/redux/HR/HRSlice";
...
console.log(location)
  ...
  const defaultData = {
    ...
    threshold:location?.threshold*1,
    daysToReport:location?.daysToReport*1,
    escalate:Boolean(location?.escalate),
    probablePunishments:location?.probablePunishments?.map((x, index)=>{
  ...
  const submit = (data) => {
    ...
    // return;
   console.log(location)
    dispatch(UpdateQuerySetup(payload))?.then(
      (res)=>
        {
  ...
  return (
    <PageLayout title={"Update Query Setup"} hasBack={true}>
              ...
              <div>
                ...
                <FormSelect
                  ...
                  array={Severity?.map((x, index) => 
                    //console.log(x)
                    (
                    <option key={index} value={x?.id} selected={location?.severity === x?.name} >
                      {x?.name}
                    </option>
                 )
                  )}
