...
import { GetAllTrainingType, UpdateTrainingBeneficiary, UpdateTrainingRequisition } from "../../../../../utils/redux/HR/HRSlice";

function AddProcRequisition() {
  const dispatch = useDispatch();
  const location = useLocation()?.state;
  const { departments, isSubmitting } = useSelector((x) => x?.procurement);
  const [setTrainingTypeItems] = useState({});
  const [vendorItems, setVendorItems] = useState({});
  const { all_vendors, employees, all_branches } = useSelector((state) => state?.vendor);
  const { all_training_types } = useSelector((x) => x?.hr);
  ...
  const defaultData = {
    trainingType: location?.trainingTypeId,
    ...
    beneficiaries: location?.trainingBeneficiaries || [
      {
        branchName: "",
        departmentName: "",
        branchId: "",
  ...
  const submit = (data) => {
    const totalData = {
      ...data,
      TrainingId: location?.uuId,
      StreetAddress2: location?.streetAddress2  === "null" ? "" :  location?.streetAddress2,
      vendorName: vendorItems?.businessName || location?.vendorName,
    ...
    dispatch(UpdateTrainingRequisition(fileData))?.then((res) => {
        if (res?.payload?.successful === true) {
            if(totalData?.beneficiaries?.length > 0){
                ...
                dispatch(UpdateTrainingBeneficiary(trainingBeneficiary))?.then((res2) => {
            ...
            <div className={DashboardStyle.inputs_group}>
              <div style={{ gridTemplateColumns: "1fr" }}>
                <FormSelect
                  ...
                  disabled={true}
                    array={all_training_types?.map?.((x, index) => (
                    ...
                    moreRegister={{
                      onChange: (e) => {
                        const isFound = all_training_types?.find(
    ...
    <>
      <div>
        ...
        {flip === "1" && (
          <FormSelect
            title={"Department"}
            camelCase={`${fieldName}.departmentName`}
            placeholder={"select"}
        ...
        {flip === "2" && (
          <FormSelect
            title={"Branch"}
            camelCase={`${fieldName}.branchName`}
            placeholder={"select"}
            // isOptional={true}
            // disabled={true}
