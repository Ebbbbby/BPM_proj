...
import { GetOvertimeType,DeleteOvertimeType,CreateOvertimeType, UpdateOvertimeType } from '../../../../../utils/redux/HR/HRSlice';
...
function QuerySetup() {
    ...
    const { get_overtime_type, isLoading } = useSelector((state) => state?.hr);
    ...
    overTimeTypes = overTimeTypes?.filter(row => row.title.toLowerCase().includes(searchQuery.toLowerCase()));
              ...
              {[
                  {
                    ...
                    action: () => {
                        setIsOpen(!isOpen);
                        navigate(`?modal_type=setup&isEdit=true&title=${title}&des=${description}&weight=${weight}&hours=${maximumHours}&approval=${monthlyApprovalDeadlineDay}&id=${id}`);
                    },
                  },
                  {
  ...
  export function CreateSetUpActions({
      ...
      const submit = (data) => {
          ...
          : 
          dispatch(CreateOvertimeType(data))?.then((res) => {
              if (res?.payload?.successful === true) {
                setIsOpen(!isOpen);
                dispatch(
                    GetOvertimeType({
      ...
      return (
        <div className={DashboardStyle.dash_board_home_nav}>
          <div className={DashboardStyle.dash_board_home_nav_header}>
            <h4>
              {GetSearchParams("isEdit") === "true" ? "Update" : "Create"} Overtime Type
            </h4>
            <FiX
            ...
            <div>
              <FormProvider {...formMethods}>
                <FormTemplate handleSubmit={handleSubmit(submit)}>
                  ...
                  <FormTextArea title={"Description"} camelCase={"description"} rowsLines={3} />
