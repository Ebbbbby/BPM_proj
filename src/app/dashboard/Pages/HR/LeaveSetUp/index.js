    ...
    return(
      <>
       <tr key={index}>
        <td>{index + 1}</td>
        <td>{title}</td>
        <td>{durationValue > 1 ? `${durationValue} Days`
         : `${durationValue} Day`
        }</td>
        <th>{description}</th>
        <td>
            ...
            {[
                {
                  ...
                  action: () => {
                      setIsOpen(!isOpen);
                      navigate(`?modal_type=setup&isEdit=true&title=${title}&des=${description}&duration=${durationValue}&gender=${genderEligibility}&group=${staffGroup}&id=${Id}`);
                  },
                },
...
function CreateSetUpActions  ({ 
  ...
  isLoading}){   
    ...
    const defaultData = {
      ...
      durationValue: +GetSearchParams('duration'),
      durationType: 0,
      staffGroup: parseInt(GetSearchParams('group')),
      genderEligibility: parseInt(GetSearchParams('gender')),
    };
    ...
    const submit = (data) => {
      const payload ={
        ...
        durationType:0,
        staffGroup:data?.staffGroup *1,
        genderEligibility:data?.genderEligibility *1,
      }
      ...
      console.log(payload, 'PAYLOAD')
        ...
        : 
        dispatch(CreateLeaveType(payload))?.then((res) => {
            if (res?.payload?.successful === true) {
              setIsOpen(!isOpen);
            ...
            <FormProvider {...formMethods}>
              <FormTemplate handleSubmit={handleSubmit(submit)}>
                ...
                array={staffGroup?.map((requestor, index) => (
                  <option key={index} value={parseInt(requestor?.id)}>
                    {requestor?.name}
                  </option>
                ))
              }   
