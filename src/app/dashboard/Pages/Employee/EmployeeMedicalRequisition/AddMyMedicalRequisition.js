...
import { CreateMedicalRequisition } from "../../../../../utils/redux/Employee/EmployeeSlice";
function AddMyMedicalRequisition() {
  ...
  const submit = (data) => {
    // const totalData = {
    //   ...data,
    //   medicalRequisitionBenficiaries:data?.medicalRequisitionBenficiaries?.map((x)=> { return{patientId: x *1}})    };

    // console.log(data?.medicalRequisitionBenficiaries,"")
    // const NamesOfBeneficiaries =  get_beneficiary_info?.responseObject?.pageItems?.filter?.((x) => {
   
    //   if (data?.medicalRequisitionBenficiaries?.includes?.(x?.id?.toString())) {
    //     return true;
    //   }
    //   return false;
    // });
  ...
  "medicalRequisitionBenficiaries":data?.medicalRequisitionBenficiaries?.map((x)=> { return{patientId: x *1}}),
    ...
    // data?.medicalRequisitionBenficiaries?.forEach((element,index) =>{
    //   console.log(element)
    //   fileData.append(`MedicalRequisitionBenficiaries[${index}].patientId`, +element?.id);
    // })
    ...
    dispatch(CreateMedicalRequisition(send))?.then((res) => {
      console.log(res)
      if (res?.payload?.successful === true) {
        reset();
         navigate(`../../${URL?.My_Medical_Requisition}`);
      }
    });
  };
  const documentList = [
          ...
          <section className={DashboardStyle.form_section}>
            ...
            <div className={DashboardStyle.inputs_group}>
              ...
              <div>
                ...
                {flipData === 1 && (
                  <MultiSelect                         
                  dataValues={watch()?.medicalRequisitionBenficiaries}
                    title={"Patient Name"}
                    name={"medicalRequisitionBenficiaries"}
                    placeholder={"Select"}
                    isOptional={watch()?.medicalRequisitionBenficiaries?.length !== 0 ? true : false}
...
export default AddMyMedicalRequisition;
